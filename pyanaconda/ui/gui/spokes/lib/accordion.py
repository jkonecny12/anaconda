...
class BasePage(Gtk.Box):
    ...
    def _chosen_changed(self, selector, _user_data):
        if selector.get_chosen():
            self.selected_members.add(selector)
        else:
            self.selected_members.discard(selector)

        if self.selected_members:
            label = Gtk.Label(label="""<span size='large' weight='bold' fgcolor='black'>%s (%s/%s)</span>""" %
                              (escape_markup(self.pageTitle), escape_markup(len(self.selected_members)),
                              escape_markup(len(self.members))),
                              use_markup=True, xalign=0, yalign=0.5, wrap=True)
        else: # do not show number of selected items when there are no items selected
            label = Gtk.Label(label="""<span size='large' weight='bold' fgcolor='black'>%s</span>""" %
                              escape_markup(self.pageTitle), use_markup=True,
                              xalign=0, yalign=0.5, wrap=True)

        # Set name of the page to expander
        self.get_parent().set_label_widget(label)
    ...
    def add_selector(self, device, cb, mountpoint=""):
        ...
        selector.connect("chosen_changed", self._chosen_changed, None)
